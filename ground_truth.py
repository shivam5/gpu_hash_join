...
for ifile in input_file:
    for ofile in output_file:
        d1 = pd.read_csv(f'data/benchmark/table{ifile}.csv')
        d2 = pd.read_csv(f'data/benchmark/table1M_id{ofile}.csv')
        d_result = pd.merge(d1.iloc[:, 1:], d2.iloc[:, 1:], on='id', how='inner')
        d_sorted = d_result.sort_values(by=['id', 'v2', 'v1'], ascending=[True, True, True])
        d_sorted.to_csv(f'data/benchmark/result_{ifile}_{ofile}.csv', index=False)